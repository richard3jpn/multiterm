...
## Phase 31: 実行中を青・待機をオレンジへ入れ替え、画面全体の外枠を追加（2026-09-04）

Phase 30 で赤・黄・青の3系統に整理したが、**割り当てが逆だった**という指摘を受けて入れ替えた。

| 状態 | Phase 30 | Phase 31 |
|---|---|---|
| 入力待ち blocked | 赤 red-500 + 点滅 | 変更なし |
| 実行中 working | 黄 amber-400 | **青 blue-500** |
| 待機 idle / 完了 done | 青 blue-500 | **黄 amber-400** |

エージェントを見張る使い方では、実行中は放っておけて、入力待ち（承認を求めている）と
待機（作業が終わって手が空いた）こそ気づきたい。そのため目につく色を待機へ、
落ち着いた色を実行中へ割り当て直した。

`paneFrameClasses()` を新規追加し、集約状態を画面全体の縁にも出すようにした。
`inset` shadow で内側に描くため、枠の有無でレイアウトがずれない。
入力待ちは赤3px＋内側発光、待機・完了は黄3px、実行中は枠なし（放っておいてよいため）。

コミット `5890249`。tsc -b 通過 / vitest 81件 GREEN。

## Phase 32: 待機・完了の色をオレンジへ（2026-09-04）

Phase 31 の黄（`amber-400`）を `orange-300` に変えた。より明るく淡いオレンジにして、
実行中の青との差を保ちながら主張を和らげる。

**shadow の rgba を Tailwind v4 の実値に合わせた**。v4 のパレットは oklch 定義で、
`--color-orange-300` は `oklch(83.7% 0.128 66.29)` = `rgb(255,184,106)`。
従来の `rgba(251,191,36)` は v3 時代の amber-400 の hex 流用で、v4 の実値（#ffb900）と
ズレていた。これで枠（border）と発光（shadow）が同じ色になる。

実機で computed style を確認: border `oklch(0.837 0.128 66.29)`、
shadow `rgba(255,184,106,0.7)`、ヘッダー帯 `oklab(0.837 … / 0.4)`、外枠 `rgb(255,184,106)`。

コミット `561d484`。RDD 2章・5章6項・11.3 の古い色記述（緑・シアン）もこの機会に現状へ合わせた。

## Phase 33: ウィンドウ切り替え（2026-09-04）

1画面の分割では収まらなくなったターミナルを、複数の「ウィンドウ」に分けて置けるようにした。
要件は RDD 14章に新設。コミット `a153f4b`（純関数レイヤ）と `8d7345e`（UI接続）。

### 調査してから決めたこと

**セッションはウィンドウに排他所属**とした。同じセッションを複数ウィンドウに出す案もあったが、
`TerminalPanel` は sessionId ごとに `resizeMessage(cols, rows)` を送る一方、PTYのサイズは
セッションに1つしか持てない。別々の大きさのペインへ同時に出すと後勝ちになり、
負けた側の表示が崩れる。両方を正しく描く方法がないため却下した。

**非アクティブウィンドウは `display:none` でDOMに残す**。外すとWebSocketが切れて再接続と
ちらつきが起き、さらにサイドバーの状態表示が各パネルのWS経由でしか届かないため、
「他ウィンドウの入力待ちを見落とさない」という要件を満たせなくなる。

実ライブラリを読んで確認した:

- `@xterm/addon-fit` の `proposeDimensions()` は、`display:none` の子孫で
  `getComputedStyle().height` が `"auto"` を返すため `parseInt` が NaN になり、
  `fit()` 冒頭の `isNaN` ガードで early return する（壊れない）
- xterm 6.0 本体は screen 要素に `IntersectionObserver` を張っており、`display:none` で
  描画を自動停止する（`_isPaused`）。WebGLのdraw callも止まるので残すコストは小さい
- `visibility:hidden` / `opacity:0` はこの検知が働かず描画され続けるため選べない

### 最大の落とし穴（実装前に発見）

起動時、非アクティブウィンドウの端末は `display:none` の中でマウントされる。
`fit()` が no-op なので `cols/rows` は既定の 80×24 のまま。ここで
**`ws.onopen` が `resizeMessage(80, 24)` を無条件に送り、PTYを80桁に潰す**。
実行中のTUIが破壊的に再フローする。

ガードを3か所に入れた:

1. `ws.onopen`: 表示中のみ送る
2. `ResizeObserver`: `display:none` の 0×0 発火では何もしない
3. 表示に戻ったとき: `requestAnimationFrame` を挟んで測り直して送る
   （同フレームでは親のサイズが取れない）。`proposeDimensions()` が NaN の間は送らない

RO と rAF は両方要る。ROは「隠れている間にサイドバー幅やブラウザサイズが変わった」場合を拾い、
rAFは再表示時のサイズが隠す直前と同一でROが発火しないケースを保証する。

### 追加・変更したファイル

| ファイル | 内容 |
|---|---|
| `features/window/window-model.ts` | ウィンドウの型と非破壊操作。名前の自動採番は空き番号を埋める |
| `features/window/build-windows.ts` | 生存セッションとの突き合わせ。重複は先に出た方を残す |
| `features/window/window-persistence.ts` | `multiterm.layout.v2` の読み書きとv1からの移行 |
| `features/window/window-keys.ts` | Alt+数字 / Alt+Shift+数字 の判定 |
| `features/window/close-window.ts` | 閉じるときの確認判定と文言 |
| `hooks/use-workspace-windows.ts` | 状態と永続化。`features/` に置くとカバレッジ閾値の対象になるため `hooks/` へ |
| `components/WindowView.tsx` | 1ウィンドウ分の描画 |
| `components/Sidebar.tsx` | `items` → `groups`。ウィンドウ見出し・追加ボタン・見出しの改名 |
| `layout/layout-tree.ts` | `isValidNode` を `isLayoutNode` として移設（型の定義元へ） |
| `layout/build-layout.ts` | `appendSessionsRight` を切り出し（`buildWindows` と共有） |

### 決めた仕様

- **v2が存在するときは、壊れていてもv1へ戻らない**。戻すとユーザーが閉じたウィンドウが復活する
- 移行はv2の保存に成功したときだけv1を削除する（保存できない環境でデータを失わない）
- サイドバーは全ウィンドウを並べる。絞り込むと他ウィンドウの入力待ちを見落とす
- Alt+数字の序数はアクティブウィンドウの行にだけ振る（他ウィンドウに出すと嘘になる）
- **Ctrl+数字は使わない**。ブラウザがタブ切り替えに予約していて `preventDefault()` で奪えない
- ウィンドウを閉じると中のセッションも終了。実行中・入力待ちを含むときだけ確認する
- 空になったウィンドウも残す（ユーザーが作った面を勝手に消さない）

### 実機で確認したこと

- リロードで2ウィンドウが復元され、非表示側もDOMに残る（`display:none`）
- 非表示のままWSが開いてもPTYが潰れない（`$Host.UI.RawUI.WindowSize` が 94×58）
- 2往復しても切断メッセージが出ず、出力も残る（WSが維持されている）
- 非表示ウィンドウで実行中のとき、見出しのドットが青（`bg-blue-500`）になり、
  上部の集計に「実行中 1」と数えられる ← 要件の核心
- v1しかない状態から起動すると Window 1 に包まれて移行し、v1が消える
- ウィンドウを閉じるとバックエンドのセッションも消える（`/api/sessions` が1件に）
- 最後の1ウィンドウでは閉じるボタンが出ない

### 既知の制約

Chromeは1ページあたりのWebGLコンテキストを約16個に制限する。セッション上限16と一致するため、
上限まで開くと古いコンテキストが失効しうる。`webgl.onContextLoss` でDOMレンダラへ
フォールバックするため壊れはしない（単一ウィンドウで16分割した場合も同じ）。

### テスト

81件 → 140件（+59）。tsc -b 通過、oxlint は既存の警告2件のみ。

## Phase 34: Alt+数字の序数を全ウィンドウ通しへ（2026-09-07）

ウィンドウごとに1から振り直していた Alt+数字の序数を、全ウィンドウ通しの番号に変えた。
**Phase 33 で決めた「アクティブウィンドウの行にだけ番号を振る」を覆している。**

### なぜ覆したか

ウィンドウごとに1から振ると、Window 1 の1番目も Window 2 の1番目も Alt+1 になる。
番号が衝突していて、押したときにどちらへ飛ぶかが仕様として決まらない。Phase 33 は
「数字はいま見ているウィンドウのN番目を指す」と定義してこれを回避したが、そうすると
他ウィンドウのターミナルへキーボードで直接飛べない。通し番号なら衝突せず、
見ていないウィンドウのターミナルへも1打で移動できる。

### 変更したもの

| ファイル | 変更 |
|---|---|
| `features/window/window-model.ts` | `MAX_DIGIT_ORDINAL = 9` と `buildDigitOrdinals()` を追加 |
| `components/Workspace.tsx` | Alt+数字を `collectAllSessionIds(windows)` 引きへ。サイドバーの序数を `buildDigitOrdinals` の結果へ |
| `components/WindowView.tsx` | 序数を親から `ordinals` で受け取る（ウィンドウ内での再計算をやめた） |

### 追加実装が要らなかったところ

`collectAllSessionIds`（全ウィンドウのID列）と、所属ウィンドウへの切り替えを内包した
`focusSession` が Phase 33 の時点で既にあった。ハンドラの引き先を差し替えるだけで
「他ウィンドウのペインへ飛ぶとウィンドウも切り替わる」が成立する。

### 決めた仕様

- 序数の順序はウィンドウの並び順 → 各ウィンドウ内の視覚順
- Alt+数字は1〜9しかないので10個目以降には番号を割り当てない（セッション上限は16）
- サイドバーの序数は全ウィンドウの行に出す（Phase 33 は非アクティブ側を null にしていた）
- Alt+Shift+数字（ウィンドウ切り替え）は変更なし

### 実機で確認したこと

Window 1 に Terminal 1・2、Window 2 に Terminal 3 を置いた状態で:

- サイドバーの番号が 1 / 2 / 3 と通しで振られ、非アクティブな Window 2 の行にも 3 が出る
- Alt+1（Window 2 を見ている状態）で Window 1 へ切り替わり Terminal 1 がアクティブになる
- Alt+3（Window 1 を見ている状態）で Window 2 へ切り替わり Terminal 3 がアクティブになる
- Alt+Shift+1 で Window 1 へ切り替わる（既存動作のまま）

### ビルド環境について分かったこと

`scripts/build-frontend.ps1` の ASCIIパス退避（`C:\Temp\multiterm-frontend-build` への
robocopy 往復）は、**このPCでは不要になっている**。旧PCではプロジェクトが OneDrive 配下の
日本語・全角中黒を含むパスにあったが、移行後は `C:\Users\hirokiasano\multiterm` で全てASCII。

`frontend` 直下で直接ビルドした生成物と、退避してビルドした既存 `dist` を比べたところ、
Vite のコンテンツハッシュを含むファイル名とサイズが完全に一致した
（`index-Bq6M45_n.js` 513,739 bytes / `index-DANIKykK.css` 30,105 bytes）。
未解決の bare import も無く、テストも `frontend` 直下で 140件 GREEN になる。

退避処理そのものの撤去は別途。

### テスト

140件 → 144件（+4）。tsc -b 通過、oxlint は既存の警告2件のみ。

## Phase 35: ウィンドウ内のペイン配置変更（2026-09-07）

分割したあとに位置を変えられるようにした。要件は RDD 15章に新設。

### レイアウトツリーに足したもの

`layout-tree.ts` には分割・削除・比率変更はあったが、**既にある葉を別の位置へ移す操作が無かった**。
`moveLeaf(node, sessionId, targetSessionId, position)` を足した。

**元の位置から取り除いてから挿し込む**順序にしている。逆にすると、移動先が
「移動するペインを含む分割ノード」だったときに、挿し込んだ直後の木へ削除をかけて
新しく置いた葉まで消す。

動かせない場合（同じペインへ落とした・移動先が無い・そのペインしか無い）は
元の木をそのまま返す。呼び出し側で成否を分岐させず、常に木を受け取れるようにした。

### 落とす位置の決め方

`resolveDropPosition(width, height, offsetX, offsetY)` で4辺のうち最も近い辺を選ぶ。
**辺までの距離はペインの幅・高さで正規化する。** 正規化しないと、横長のペインでは
上下の辺が常に近くなり、左右がほとんど選べない。

中央付近でも必ずいずれかの辺に倒れる（「落とせない場所」を作らない）。
距離が並んだときは left → right → top → bottom の順で決め、結果を決定的にした。

### ドラッグの実装で決めたこと

- **ドラッグできるのはヘッダーだけ**。端末の中身を対象にすると xterm のテキスト選択が死ぬ
- 名前を編集している間は `draggable` を外す（テキスト選択と競合する）
- 落とす先はパネル全体。ヘッダーだけにすると上下の判定が実用にならない
- ドラッグ中のペインは独自MIME `application/x-multiterm-session` で運ぶ。
  `text/plain` だとエディタやブラウザから流れてきた文字列まで拾う。
  独自の型なら `dragover` の時点で `dataTransfer.types` を見るだけで判別できる
  （`getData` は drop まで読めない仕様）
- `dragleave` は子要素へ移っただけでも飛ぶため、`currentTarget.contains(relatedTarget)` で
  外へ出たときだけハイライトを消す

ウィンドウをまたぐ移動は RDD 14.7 のスコープ外のまま。所属が違うドロップは黙って無視する。

### 実機で確認したこと

Terminal 1・2 が左右に並んだ状態で、Terminal 2 のヘッダーを Terminal 1 へドラッグ:

- localStorage が `first: 339cd85f(T2), second: 5454ddbc(T1)` へ入れ替わった
- 画面も左が Terminal 2、右が Terminal 1 になった
- **Alt+数字の序数が視覚順に追従した**（Terminal 2 が「1」、Terminal 1 が「2」）。
  Phase 34 の通し番号と噛み合っている

### テスト

144件 → 157件（+13）。移動4方向・深い階層・集合の不変・動かせない3条件・
落とす位置の正規化・タイの決定性・0除算。

## Phase 36: デスクトップアプリ化（2026-09-07）

ブラウザのタブではなく独立した窓で開けるようにした。要件は RDD 16章に新設。

### 別バイナリにした理由

WebView（wry）は Linux で gtk / webkit2gtk を引き込む。1つのバイナリに同居させると、
GUIの要らない環境でサーバをビルドするだけで開発パッケージ一式が要る。

Cargo workspace に分け、サーバ本体を `multiterm-backend` のライブラリ側（`lib.rs` の `serve`）へ移した。

| バイナリ | 役割 |
|---|---|
| `multiterm-backend` | サーバだけ。画面はブラウザ。従来通り |
| `multiterm-app` | 同じプロセスでサーバを動かし、WebViewの窓も開く |

`cargo build --package multiterm-backend` では app 側の依存を一切引かない。
app がサーバを子プロセスとして起動しないのは、プロセスを1つに保ち、
常駐メモリの小ささ（RDD 3章）をそのまま活かすため。

**workspace 化で `[profile.release]` はルートへ移した**（メンバー側の profile は無視される）。

### 起動と終了の順序

- **バインドが済んでから窓を開く。** `serve` はバインド直後にコールバックを呼び、
  app 側は `mpsc` でそれを待ってから WebView を作る。先に開くと真っ白な画面になる
- tao のイベントループはメインスレッドでしか回せないため、tokio ランタイムは別スレッドへ置いた
- **`run` ではなく `run_return` を使う。** `run` はそのままプロセスを終わらせてしまい、
  サーバスレッドの `join` を待てない。窓を閉じたあとに PTY の後始末を待つ必要がある

### 実測値

| 項目 | 値 |
|---|---|
| `multiterm-app.exe` | 5,207,552 bytes |
| `multiterm-backend.exe` | 4,638,208 bytes |
| 差（wry + tao 分） | 569 KB |
| release ビルド時間 | 1分23秒 |
| 起動直後の Private メモリ | 5.8 MB（WebView2 のプロセス群は別勘定） |

### 実機で確認したこと

- `MainWindowTitle` が "MultiTerm"、`MainWindowHandle` が非0 → 窓が開いている
- **同じPID（15700）がポート3001を listen** → サーバと窓が1プロセスに同居している
- `GET /api/health` と `POST /api/sessions` が窓から動く
- **ウィンドウを閉じる（WM_CLOSE）と、PTYのPowerShell（PID 33308）も消えた**。
  ポート3001も解放。`run_return` → `join` の順序が効いている

### ConPTY についての発見

`Win32_Process` で app の直接の子を見ると **PowerShell が居ない**。
ConPTY ではシェルがバックエンドの直接の子として現れない。
Phase 37 のリソース計測で「自分の子孫を辿るだけ」にしなかったのは、これが理由。

## Phase 37: リソースモニタ（2026-09-07）

アプリ全体とターミナルごとの CPU・メモリを画面に出した。要件は RDD 17章に新設。

### 測り方

sysinfo 0.39 でプロセスツリーを辿る。`Process::parent()` は上向きにしか辿れないので、
毎回 親PID → 子PID のマップを組み直して下向きに辿る。

**アプリ全体は「自分のツリー」と「各ターミナルのツリー」の和集合で測る。**
Phase 36 で分かった通り、ConPTY ではシェルが直接の子として現れない。
自分の子孫を辿るだけだとターミナル分が丸ごと抜ける。
根が親子関係にある場合の二重計上は、訪問済みPIDを飛ばして防ぐ。
PIDの使い回しで親子が循環していても、同じ理由で無限ループにならない。

シェルのPIDは **セッション生成時に一度だけ** 取り出して `PtyHandle` に持たせた。
`child` から都度読むと、出力の多いセッションで PTY の読み書きと Mutex を奪い合う。

### CPU使用率の扱い

- **`System` は持ち回す。** 呼ばれるたびに作り直すと差分が取れず常に 0% になる
- 画面からは2秒間隔で取りにいく。空けすぎると平均化されて動きが見えず、
  詰めすぎると全プロセスの走査が頻繁になって測っている側が重くなる
- 全プロセスの走査はミリ秒単位で待たされるので `spawn_blocking` で逃がす
- 表示は**論理コア数で割ってマシン全体に対する割合**にする。バックエンドが返すのは
  1コア=100%の値で、14コアの機械で1コア使い切ると 100 になるが、体感は「全体の7%」

### 実測値（このPC・14コア）

```
app     : cpuPercent 5.7 / memoryBytes 212,385,792
session1: cpuPercent 0.0 / memoryBytes  81,022,976
session2: cpuPercent 0.0 / memoryBytes  80,932,864
cpuCount: 14
```

画面表示はヘッダーが `CPU 0.1% · 203 MB`、サイドバーの各行が `0.0% · 77 MB`。
バックエンド単体が 16MB、PowerShell が1つ 77〜81MB という内訳になる。

### テスト

157件 → 164件（+7）。CPU使用率の換算・0除算・小数1桁の表示・
メモリのKB/MB/GB切り替え・負値のガード。

## Phase 38: タイトルバーを消してアイコンを付ける（2026-09-07）

デスクトップ版でOSのタイトルバーを出すのをやめ、アプリのヘッダーがその役目を兼ねるようにした。
要件は RDD 16.6〜16.8。

### なぜ消すか

アプリのヘッダーが既にあり、その上にOSの帯が乗ると二重になって収まりが悪い。
ただし装飾を消すと、窓の移動・リサイズ・最小化・最大化・閉じるが全部なくなる。
そこで画面側から IPC で受けて、ネイティブ側で実行する。

| 操作 | 実現方法 |
|---|---|
| 移動 | ヘッダーの空白をドラッグ。CSS の `app-region: drag`（WebView2 123+）が効き、それ未満は `mousedown` を IPC で送って `drag_window()` |
| リサイズ | 窓の枠5px以内で `mousedown` → `drag_resize_window()`。どの辺かはネイティブ側で判定 |
| 最小化・最大化・閉じる | ヘッダー右端のボタンから IPC |

### 公式サンプルから変えた2点

**1. `mousemove` を毎回送らない。** wry の `custom_titlebar` 例は
`document.addEventListener('mousemove', ...)` で全部の動きを IPC へ流す。
ターミナルを操作している間ずっとIPCが飛び続けるため、枠の近くに居る間と、
離れた直後の1回（変わったカーソル形状を戻すため）だけに絞った。

**2. ドラッグ領域にボタンを1つも含めない。** `app-region: drag` の中にある
インタラクティブ要素は `no-drag` を振らないと反応しなくなる。ヘッダーを
「タイトル・カウンタ・使用量」と「操作ボタン群」に分け、前者だけをドラッグ領域に
することで、個々の要素へ `no-drag` を振る必要をなくした。

枠と見なす幅（5px）はネイティブ側と画面側の両方に定数がある。片方だけ変えても、
画面側が IPC を送らなければカーソルは変わらない。両方に「同じ値にすること」と書いた。

### 窓の操作UIはデスクトップ版だけ

`window.ipc` の有無で判定する。ブラウザで開いたときはボタンもドラッグ領域も出さない。
判定はモジュール読み込み時に一度だけ（起動後に変わることはない）。

### 検証で分かったこと

`GetWindowLong` で見ると **`WS_CAPTION` ビットは立ったまま**だった。
tao は Windows のリサイズ・スナップ機能を保つためにスタイルを残し、
`WM_NCCALCSIZE` でクライアント領域を上端まで広げる実装になっている。
スタイルビットでは判定できないので、`GetWindowRect` と `ClientToScreen` の差で確かめた:

```
ウィンドウ矩形 : Top=491
クライアント領域: Top=492
上端の差: 1px（枠線のみ = タイトルバーなし）
```

### アイコン

`frontend/public/favicon.ico`（7サイズ入り・201KB）を `winresource 0.1.31` で
実行ファイルへ埋め込んだ。Windows ではタスクバー・エクスプローラー・窓のアイコンが
すべて exe のリソースから取られるため、これだけで揃う。
`ExtractAssociatedIcon` で 32x32 を取得できることを確認。
exe は 5,207,552 → 5,427,712 bytes（+220KB ≒ ICO のサイズ）。

埋め込みに失敗してもビルドは止めない（アイコンが無くてもアプリは動く）。

### 状態の外枠を作り直した

画面全体を囲んでいた枠を、**ヘッダーより下のターミナル領域だけ**に変えた。
ヘッダーがタイトルバーを兼ねるようになり、窓の縁と枠が重なって見苦しかったため。
太さも 3px → 5px にした。

`paneFrameClasses` のテストは色と `inset` の有無だけを見ていたので、太さの変更で
落ちるテストは無かった。
