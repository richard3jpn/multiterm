...
- 入力待ち（waiting-input）: 赤い境界線＋強い発光
- 実行中（running）: 青い境界線
- 待機（idle）: オレンジの境界線＋発光

ペインの枠だけでなく、ターミナル領域全体の外枠も最も注意が必要な状態の色で縁取る（入力待ち=赤 / 待機・完了=オレンジ / 実行中=枠なし）。縁取るのはヘッダーより下だけで、ヘッダー自体は囲まない（デスクトップ版ではヘッダーがタイトルバーを兼ねるため）。
...
- **コピー・ペースト**: 選択して Ctrl+C でコピー（選択が無いときの Ctrl+C は従来どおり中断の ^C）、Ctrl+V で貼り付け
...
| 表示 | 意味 |
|---|---|
| 赤（点滅） 入力待ち | 入力・承認を待っている。すぐ対応が必要 |
| 青 実行中 | コマンドが動いている |
| オレンジ 完了（未確認） | 実行が終わったが、そのターミナルをまだ見ていない |
| オレンジ 待機 | 確認済み、または最初から待機 |

完了と待機は同じオレンジで、色では区別しない（数はヘッダーの集約表示で分かる）。「完了（未確認）」はそのターミナルを開くまで残る。別のペインを見ている間に終わった処理を見落とさないための表示で、行をクリックすると「待機」に戻る。ヘッダーにも最も注意が必要な状態が集約表示される。
...
**OSのタイトルバーは出さない。** アプリのヘッダーがその役目を兼ねる。ヘッダーの空白をドラッグすると窓が動き、窓の枠（5px以内）でリサイズでき、右端の最小化 / 最大化 / 閉じるボタンが使える。ブラウザで開いたときはこれらのUIは出ない。タスクバーのアイコンは `frontend/public/favicon.ico` を exe に埋め込んで使う。
...
ヘッダーの `v0.7.0` は `frontend/package.json` の `version` をビルド時に埋め込んだもの
（`vite.config.ts` の `define`）。**どのビルドを動かしているか見分けるための表示なので、
見た目や挙動が変わる変更を入れたら手で上げること。** 上げ忘れると別のビルドが同じ番号になる。
